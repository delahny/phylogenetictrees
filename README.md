...
### Running the Script
At minimum, this script requires an excel sheet containing 2 columns named: "Genome_Change" and "ID". The script iterates through the Genome_Change column to find reptitions and creates subsets of clonal relationships based on its occurence within each subset. Therefore, the input sheet should be a concatenation of somatic mutations (rows) from the set of clonally related cells, and each mutation (row) should have a corresponding sample ID from which it was extracted. 
