...
8. Only cells with known overlaps, as determined by this pipeline, should be used to generate a spreadsheet compiling mutations for those cells. For example, if 10 cells are sequenced and mutation overlaps are identified between cells 1 and 2, as well as between cells 3 and 4, two separate spreadsheets should be created: one for cells 1 and 2, and another for cells 3 and 4. These spreadsheets will then serve as separate inputs for the R script.
